 ...
 * @desc 現在のターン数を代入するゲーム変数番号：将当前回合数代入游戏变量编号
 * 初期値: 1
 * @default 1
 *
 * @param vnActorUnitNums
 * @desc 残りアクターユニット数を代入するゲーム変数番号：将剩余动作单元数代入游戏变量编号
 * 初期値: 2
 * @default 2
 *
 * @param vnEnemyUnitNums
 * @desc 残りエネミーユニット数を代入するゲーム変数番号：代入剩余单元数的游戏变量编号
 * 初期値: 3
 * @default 3
 *
 * @param srpgBasicWait
 * @desc 敵ターン進行の基本ウェイト時間（フレーム）：敌人回合进行的基本重量时间(帧)
 * 初期値: 30
 * @default 30
 *
 * @param damagePopupInterval
 * @desc 連続ダメージポップアップの間隔（フレーム）：连续伤害弹出的间隔(帧)
 * 初期値: 30
 * @default 30
 *
 * @param checkSkillId
 * @desc 調べる/話すスキルの番号：调查/说话技能的号码
 * 初期値: 0
 * @default 0
 *
 * @param simpleMoveInput
 * @desc 移動可能範囲が表示中ならそのまま移動できるようにする：如果显示可移动范围，就可以直接移动。
 * 初期値: 0 ( 0 = 無効：无效/ 1 = 有効：有效)
 * @default 0
 *
 * @param moveCommand
 * @desc 移動コマンド名：移动命令名
 ...
 *
 * @param itemCommand
 * @desc アイテムコマンド名（未入力でコマンド削除）：项目命令名(未输入命令删除)
 * 初期値: アイテム：条款
 * @default アイテム：条款
 *
 * @param equipCommand
 * @desc 装備変更コマンド名（未入力でコマンド削除）：装备变更命令名(未输入命令删除)
 * 初期値: 装備変更
 * @default 装備変更
 *
 * @param waitingCommand
 * @desc 待機コマンド名（未入力でコマンド削除）：待机命令名(未输入命令删除)
 * 初期値: 待機
 * @default 待機
 *
 * @param statusCommand
 * @desc ステータスコマンド名（未入力でコマンド削除）：状态命令名(未输入命令删除)
 * 初期値: ステータス：状态
 * @default ステータス：状态
 *
 * @param turnEndCommand
 * @desc ターン終了コマンド名：回合结束命令名
 * 初期値: ターン終了：回合结束
 * @default ターン終了：回合结束
 *
 * @param unitListCommand
 * @desc ユニットリストコマンド名（未入力でコマンド削除）：单元列表命令名(未输入命令删除)
 * 初期値: ユニットリスト：单元列表
 * @default ユニットリスト：单元列表
 *
 * @param infoCommand
 * @desc マップ情報コマンド名（未入力でコマンド削除）：地图信息命令名(未输入命令删除)
 * 初期値: マップ情報：地图信息
 * @default マップ情報：地图信息
 *
 * @param optionsCommand
 * @desc オプションコマンド名（未入力でコマンド削除）：选项命令名(未输入命令删除)
 * 初期値: オプション：选项
 * @default オプション：选项
 *
 * @param saveCommand
 * @desc セーブコマンド名（未入力でコマンド削除）：保存命令名(未输入命令删除)
 * 初期値: セーブ：保存
 * @default セーブ：保存
 *
 * @param loadCommand
 * @desc ロードコマンド名（未入力でコマンド削除）：加载命令名(未输入命令删除)
 * 初期値: ロード：加载
 * @default ロード：加载
 *
